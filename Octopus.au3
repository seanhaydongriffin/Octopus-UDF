...
#Region Header
#cs
	Title:   		Octopus UDF Library for AutoIt3
	Filename:  		Octopus.au3
...
Global Const $sap_vkey[100] = [ "Enter", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", _
								...
								"", "", "", "", "", "", "", "Shift+F10", "", "", "", "", "" ]
Global $octopus_domain = ""
Global $octopus_api_key = ""
Global $testrail_password = ""
Global $testrail_encrypted_password = ""
...
;
; ;==========================================================================================
Func _OctopusDomainSet($domain)

	$octopus_domain = $domain
EndFunc
...
Func _OctopusGetTask($task_id)

	Local $iPID = Run('curl.exe -k -X GET -H "accept: application/json" -H "X-Octopus-ApiKey: ' & $octopus_api_key & '" ' & $octopus_domain & '/api/tasks/' & $task_id & '/details', @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octopus_json = StdoutRead($iPID)
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;	Exit
...
Func _OctopusGetTaskState($task_id)

	Local $output = ""

	_OctopusGetTask($task_id)
	Local $decoded_json = Json_Decode($octopus_json)
	$output = Json_Get($decoded_json, '.Task.State')
	Return $output
...
Func _OctopusGetEnvironments()

	Local $iPID = Run('curl.exe -k -X GET -H "accept: application/json" -H "X-Octopus-ApiKey: ' & $octopus_api_key & '" ' & $octopus_domain & '/api/environments/all', @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octopus_json = StdoutRead($iPID)
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _OctopusGetTenant($tenant_id)

	Local $iPID = Run('curl.exe -k -X GET -H "accept: application/json" -H "X-Octopus-ApiKey: ' & $octopus_api_key & '" ' & $octopus_domain & '/api/tenants/' & $tenant_id, @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octopus_json = StdoutRead($iPID)
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _OctopusGetTenants()

	Local $iPID = Run('curl.exe -k -X GET -H "accept: application/json" -H "X-Octopus-ApiKey: ' & $octopus_api_key & '" ' & $octopus_domain & '/api/tenants/all', @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octopus_json = StdoutRead($iPID)
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _OctopusGetTenantsIDName()

	Local $output[0][2]

	_OctopusGetTenants()

	$octopus_json = "{""Items"":" & $octopus_json & "}"
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	Local $decoded_json = Json_Decode($octopus_json)

	for $j = 0 to 99999

		Local $id = Json_Get($decoded_json, '.Items[' & $j & '].Id')

		if @error > 0 Then ExitLoop

		Local $name = Json_Get($decoded_json, '.Items[' & $j & '].Name')
...
Func _OctopusGetTenantName($tenant_id)

	Local $output = ""
	_OctopusGetTenant($tenant_id)
	Local $decoded_json = Json_Decode($octopus_json)
	$output = Json_Get($decoded_json, '.Name')
	Return $output
EndFunc
...
Func _OctopusGetDeployments($environments = "")

	if StringLen($environments) > 0 Then

		$environments = "&environments=" & $environments
	EndIf

	Local $iPID = Run('curl.exe -k -X GET -H "accept: application/json" -H "X-Octopus-ApiKey: ' & $octopus_api_key & '" ' & $octopus_domain & '/api/deployments?take=999' & $environments, @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octopus_json = StdoutRead($iPID)
	;ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _OctopusGetLatestTenantIdsDeploymentIdTaskIdCreatedForEnvironment($environment)

	Local $output[0][4]
	Local $tenant_id_dict = ObjCreate("Scripting.Dictionary")

	_OctopusGetDeployments($environment)
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octopus_json = ' & $octopus_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	for $j = 0 to 99999
		...
		Local $tenant_id = Json_Get($decoded_json, '.Items[' & $j & '].TenantId')
		Local $created = Json_Get($decoded_json, '.Items[' & $j & '].Created')
		$created = __OctopusCreatedToTimeAgo($created)
		Local $task_id = Json_Get($decoded_json, '.Items[' & $j & '].TaskId')

		if $tenant_id_dict.Exists($tenant_id) = False Then

			$tenant_id_dict.Add($tenant_id, $tenant_id)
			_ArrayAdd($output, $tenant_id & Chr(28) & $id & Chr(28) & $created & Chr(28) & $task_id, 0, Chr(28), @CRLF, 1)
		EndIf
	Next
...
Func __OctopusCreatedToTimeAgo($created)

;	$created = "2019-11-15T01:59:34.453+00:00"
	$year = StringLeft($created, 4)
	$month = StringMid($created, 6, 2)
	$day = StringMid($created, 9, 2)
	$hour = StringMid($created, 12, 2)
	$minute = StringMid($created, 15, 2)
	$second = StringMid($created, 18, 2)
	$millisecond = StringMid($created, 21, 3)
	Local $tSystem = _Date_Time_EncodeSystemTime($month, $day, $year, $hour, $minute, $second, $millisecond)
	Local $tLocal = _Date_Time_SystemTimeToTzSpecificLocalTime($tSystem)
	Local $local_str = _Date_Time_SystemTimeToDateTimeStr($tLocal, 1)
	Local $time_diff = ""
	Local $unit = ""

	$time_diff = _DateDiff("n", $local_str, _NowCalc ())
	...
	if $time_diff < 60 Then
		...
		$time_diff = $time_diff & $unit
	Else

		$time_diff = _DateDiff("h", $local_str, _NowCalc ())
		...
		Else
			...
			$time_diff = _DateDiff("D", $local_str, _NowCalc ())
			$unit = " days ago"
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $time_diff = ' & $time_diff & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _TestRailGetProjectsIDAndNameArray()
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":\d+,.*"name":".*"', 3)
...
Func _TestRailGetSuites($project_id)
...
Func _TestRailGetSuitesId($project_id, $starts_with = "")

	Local $output[0]

	_TestRailGetSuites($project_id)

	$octpous_json = "{""suites"":" & $octpous_json & "}"
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	Local $decoded_json = Json_Decode($octpous_json)

	for $j = 0 to 99999

		Local $id = Json_Get($decoded_json, '.suites[' & $j & '].id')

		if @error > 0 Then ExitLoop

		Local $name = Json_Get($decoded_json, '.suites[' & $j & '].name')

		if StringLen($starts_with) > 0 Then

			if StringInStr($name, $starts_with) = 1 Then
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $response[2] = ' & $response[2] & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"name":"(.*)",', 3)
	Return $rr


EndFunc

Func _TestRailGetCases($project_id, $suite_id, $type_id = "")

	if StringLen($type_id) > 0 Then

		$type_id = "&type_id=" & $type_id
	EndIf

	Local $iPID = Run('curl.exe -k -H "Content-Type: application/json" -u ' & $testrail_username & ':' & $testrail_password & ' ' & $testrail_domain & '/index.php?/api/v2/get_cases/' & $project_id & '&suite_id=' & $suite_id & $type_id, @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $octpous_json = StdoutRead($iPID)
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
EndFunc
...
Func _TestRailGetCasesAutomationTestCasesSaved($project_id, $suite_id)

	Local $output[0][2]

	_TestRailGetCases($project_id, $suite_id, 1)

	$octpous_json = "{""cases"":" & $octpous_json & "}"
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	Local $decoded_json = Json_Decode($octpous_json)

	for $j = 0 to 99999

		Local $id = Json_Get($decoded_json, '.cases[' & $j & '].id')

		if @error > 0 Then ExitLoop

		Local $test_cases_saved = Json_Get($decoded_json, '.cases[' & $j & '].custom_test_cases_saved')

		_ArrayAdd($output, $id & Chr(28) & $test_cases_saved, 0, Chr(28), @CRLF, 1)
	Next
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $response[2] = ' & $response[2] & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"title":"(.*)",.*"refs":"(.*)"', 3)
	Return $rr
EndFunc
...
Func _TestRailGetRunIDFromPlanIDAndRunName($plan_id, $run_name)
	...
	$rr = StringRegExp($octpous_json, '"runs":\[{"id":\d+,.*"name":"' & $run_name & '"', 1)
...
Func _TestRailGetPlanRunsID($plan_id)
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(\d+),"suite_id":\d+,"name":".*","description"', 3)
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $cmd = ' & $cmd & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	$rr = StringRegExp($octpous_json, '(?U)"defects":"(.*)","id":(.*),"status_id":(.*),"test_id":(.*),', 3)
	Return $rr
...
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $cmd = ' & $cmd & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	filewrite("D:\dwn\fred.txt", $octpous_json)

	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"test_id":(.*),.*"status_id":(.*),.*"defects":(.*),"custom_step_results"', 3)
	Return $rr
...
;	filewrite("D:\dwn\fred.txt", $octpous_json)
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"test_id":(.*),.*"status_id":(.*),.*"created_on":(.*),.*"defects":(.*),"custom_step_results"', 3)
	Return $rr
...
;	filewrite("D:\dwn\fred.txt", $octpous_json)
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"case_id":(.*),.*"run_id":(.*),.*"title":"(.*)",', 3)
	Return $rr
...
;	_TestRailGetTests($run_id)
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":\d+,.*"title":".*"', 3)
...
;	_TestRailGetTests($run_id)
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":\d+,.*"refs":".*"', 3)
...
Func _TestRailGetResultsElapsedForRuns($run_ids, $created_after, $created_before)
	...
	for $i = 0 to (UBound($run_id) - 1)
		...
		$octpous_json = "{""results"":" & $octpous_json & "}"
		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		Local $decoded_json = Json_Decode($octpous_json)
...
Func _TestRailGetCreatedOnTestSavedRuntimeForRuns($project_id, $run_suite, $created_after, $created_before, $testcase_complexity_execution_weightage_dict)
	...
	for $i = 0 to (UBound($run_suite) - 1)
		...
		$octpous_json = "{""results"":" & $octpous_json & "}"
;		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		Local $results_decoded_json = Json_Decode($octpous_json)

		if StringLen($octpous_json) > StringLen("{""results"":[]}") Then

			_TestRailGetTests($run_suite[$i][0])
			$octpous_json = "{""tests"":" & $octpous_json & "}"
			Local $tests_decoded_json = Json_Decode($octpous_json)

			_TestRailGetCases($project_id, $run_suite[$i][1])
			$octpous_json = "{""cases"":" & $octpous_json & "}"
			Local $cases_decoded_json = Json_Decode($octpous_json)
		EndIf
...
;				ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $complexity = ' & $complexity & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
				...
				$created_on_date = StringReplace($created_on_date, "/", "-")
...
Func _TestRailGetPlansIDAndNameArray($project_id)
	...
	$octpous_json = "{""plans"":" & $octpous_json & "}"
	Local $decoded_json = Json_Decode($octpous_json)
...
Func _TestRailGetPlansRunsIDArray($plan_ids)
	...
	for $i = 0 to (UBound($plan_id) - 1)
		...
		$octpous_json = "{""plan"":" & $octpous_json & "}"
		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $octpous_json = ' & $octpous_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
Func _TestRailGetPlanIDByName($project_id, $plan_name)
	...
	$rr = StringRegExp($octpous_json, '"id":\d+,"name":"' & $plan_name & '"', 1)
	$rr[0] = StringLeft($rr[0], StringInStr($rr[0], ",") - 1)
...
;	Exit
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":(.*),.*"label":"(.*)"', 3)
	Return $rr
...
;	_TestRailGetStatuses()
	...
	$rr = StringRegExp($octpous_json, '(?U)"id":\d+,.*"label":".*"', 3)
...
Func _TestRailGetSectionNameAndDepth($project_id)
	...
	$rr = StringRegExp($octpous_json, '(?U)"name":"(.*)",.*"depth":(\d+)', 3)
...
Func _TestRailAuthenticationWithToast($app_name, $domain, $ini_filename)

	$testrail_username = IniRead($ini_filename, "main", "username", "")
	$testrail_encrypted_password = IniRead($ini_filename, "main", "password", "")

	_TestRailDomainSet($domain)

	_Toast_Set(0, -1, -1, -1, -1, -1, "", 100, 100)
	_Toast_Show(0, $app_name, "Login to TestRail ...", -300, False, True)
	...
	if stringlen($testrail_decrypted_password) = 0 or StringInStr($octpous_json, "<title>Unauthorized (401)</title>", 1) > 0 Then
		...
		While 1
			...
			if $msg = $done_button Then
				...
				if StringInStr($octpous_json, "Email/Login or Password is incorrect", 1) = 0 Then
